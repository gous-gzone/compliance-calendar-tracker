...
export default function Navbar() {
  ...
  return (
    <header className="bg-white/70 backdrop-blur-md border-b border-slate-100 sticky top-0 z-30 w-full h-20 flex items-center shadow-sm">
      <div className="w-full px-4 sm:px-8">
        <div className="flex items-center justify-between">

          {/* Mobile brand */}
          <div className="flex items-center gap-3 lg:hidden">
            <div className="w-9 h-9 bg-[#1B4F8A] rounded-xl flex items-center justify-center shadow-lg shadow-[#1B4F8A]/20">
              <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
              </svg>
            </div>
            <span className="font-black text-slate-900 text-lg tracking-tight">Compliance Tracker</span>
          </div>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${
                  isActive ? 'bg-[#1B4F8A] text-white shadow-xl shadow-[#1B4F8A]/20' : 'text-slate-400 hover:bg-slate-50'
                }`
              }
            >
